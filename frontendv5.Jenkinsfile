                ...
                script {
                    ...
                    dir('artifacts') {
                        ...
                        sshagent(['sshtoaws']) {
                            ...
                            sh "ssh -v -i /var/jenkins_home/greenworld.pem ubuntu@3.23.92.68 'rm -rf ${PROJECT_DIR}/artifacts/*'"
                            sh "scp -v -rp artifacts/* ubuntu@3.23.92.68:${PROJECT_DIR}/artifacts/"
                            sh "ssh -v -i /var/jenkins_home/greenworld.pem ubuntu@3.23.92.68 'ls -la ${PROJECT_DIR}/artifacts/'"

                            // Build the Docker image on the Docker host
                            sh "ssh -v -i /var/jenkins_home/greenworld.pem ubuntu@3.23.92.68 'cd ${PROJECT_DIR} && docker build -f Dockerfile -t ${env.DOCKER_IMAGEE}:${env.ENVIRONMENT.toLowerCase()}-frontend-${env.BUILD_NUMBER} .'"
                        ...
                        withCredentials([usernamePassword(credentialsId: 'dockerhub1', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD')]) {
                            sh """
                                ...
                                ssh -i /var/jenkins_home/greenworld.pem ubuntu@3.23.92.68 'docker push ${env.DOCKER_IMAGEE}:${env.ENVIRONMENT.toLowerCase()}-frontend-${env.BUILD_NUMBER}'
                            """
                        }
